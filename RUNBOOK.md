...
adb -H 100.74.73.114 -P 5037 install -r app/build/outputs/apk/debugNoMinify/HeliBoard_0.1.7-debugNoMinify.apk
```
...
In portrait, verify that no bar or empty band remains above the keys while idle. The Enter/action key must retain its normal icon with a small microphone badge, and the badge must be absent in password fields. Long-press comma must open keyboard settings, where the bundled light-to-dark themes can be selected.
